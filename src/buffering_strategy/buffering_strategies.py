...
class SpokenAudio(BufferingStrategyInterface):
    """
    A buffering strategy that processes audio at the end of each chunk with
    silence detection.
    ...
    def process_audio(self, websocket, vad_pipeline, asr_pipeline):
        ...
        print("chunk_length_in_bytes", chunk_length_in_bytes)
        print("len(self.client.buffer)", len(self.client.buffer))
        print("len(self.client.scratch_buffer)", len(self.client.scratch_buffer))
        # if len(self.client.buffer) > chunk_length_in_bytes:
        if self.processing_flag:
            exit(
                "Error in realtime processing: tried processing a new "
    ...
    async def process_audio_async(self, websocket, vad_pipeline, asr_pipeline):
        ...
        if vad_results[-1]["end"] < last_segment_should_end_before:
            ...
            if transcription["text"] != "":
                ...
                json_transcription = json.dumps(transcription)
                await websocket.send(json_transcription)
            self.client.on_transcription(transcription["text"])
            self.client.scratch_buffer.clear()
            self.client.increment_file_counter()
